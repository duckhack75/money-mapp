        ...
        q.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot datanapshot: dataSnapshot.getChildren()){
                    ...
                    //cálculo del total del presupuesto Casas
                    if (valida2==false){
                        Toast.makeText(Einversiones.this,"error! debe llenar 1 o más campos!",Toast.LENGTH_LONG).show();
                    }else if (valida==true){


                        //obtención de la "primary key"
                        String clave=datanapshot.getKey();
                        restarEntradasTotal(contador);//método para restar el presupuesto global
                        if(!negocios.getText().toString().isEmpty()){
